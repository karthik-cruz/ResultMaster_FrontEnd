...
const AttendanceMarks = () => {
    ...
    const sortedData = useMemo(() => {
        return allStudents?.students?.length
            ? [...allStudents.students].sort((a, b) => {
                const aMarks = a.attendanceMarks ? Number(a.attendanceMarks) : 0;
                const bMarks = b.attendanceMarks ? Number(b.attendanceMarks) : 0;
    ...
    return (
        <div className='p-6'>
            {!user?.isAdmin ? (
                user?.user?.attendanceMarks ? (
                    <div>
                        <div className='flex rounded items-center gap-4 border shadow-lg p-6 w-[500px]'>
                            <PieChart targetPercentage={user?.user?.attendanceMarks ? user?.user?.attendanceMarks : 0} />
                            <StarRatings height={"50px"} width={"50px"} rating={user?.user?.attendanceMarks ? user?.user?.attendanceMarks : 0} />
                        </div>
                        <div className='mt-4'>
                            <table style={{ width: "100%", borderCollapse: "collapse" }}>
                                ...
                                <tbody>
                                    <tr  style={{ border: "1px solid #ddd" }}>
                                        ...
                                        <td style={{ border: "1px solid #ddd", padding: "8px" }}>Attendance Marks</td>
                                        <td style={{ border: "1px solid #ddd", padding: "8px" }}>{user?.user?.attendanceMarks ? user?.user?.attendanceMarks : 0}/100</td>
                                        <td style={{ border: "1px solid #ddd", padding: "8px" }}>{getPerformanceCategory(user?.user?.attendanceMarks)}</td>
                                        <td style={{ border: "1px solid #ddd", padding: "8px" }}>
                                            <StarRatings height={"24px"} width={"24px"} rating={user?.user?.attendanceMarks ? user?.user?.attendanceMarks : 0} />
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                ...
                user?.isAdmin && limitedData?.length > 0 ? (
                    <div>
                        <p className="font-bold text-[25px] text-gray-700 mb-4">Top 10 Performer In Attendance</p>
                        <table style={{ width: "100%", borderCollapse: "collapse" }}>
                            <thead>
                                <tr style={{ backgroundColor: "#f2f2f2", border: "1px solid #ddd" }}>
                                    <th style={{ border: "1px solid #ddd", padding: "8px", textAlign: "left" }}>S.no</th>
                            ...
                            <tbody>
                                {limitedData.map((user, index) => (
                                    <tr className={`${isDark ? "text-white" : "text-black"}`} key={user._id} style={{ border: "1px solid #ddd" }}>
                                        ...
                                        <td style={{ border: "1px solid #ddd", padding: "8px" }}>{user.attendanceMarks ? user.attendanceMarks : 0}/100</td>
                                        <td style={{ border: "1px solid #ddd", padding: "8px" }}>{getPerformanceCategory(user.attendanceMarks)}</td>
                                        <td style={{ border: "1px solid #ddd", padding: "8px" }}>
                                            <StarRatings height={"24px"} width={"24px"} rating={user.attendanceMarks ? user.attendanceMarks : 0} />
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
